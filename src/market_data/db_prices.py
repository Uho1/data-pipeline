"""DuckDB connection manager for US prices data (market_data_prices.duckdb)."""
...
DB_PATH: Path = DATA_DIR / "market_data_prices.duckdb"

_local = threading.local()
_snapshot_lock = threading.Lock()
_snapshot_sig: tuple[int, int] | None = None
_SNAPSHOT_RE = re.compile(r"^market_data_prices_readonly_(\d+)\.duckdb(?:\..+)?$")
...
def _snapshot_path() -> Path:
    return Path(tempfile.gettempdir()) / f"market_data_prices_readonly_{os.getpid()}.duckdb"
...
def cleanup_stale_snapshot_copies() -> int:
    ...
    for candidate in temp_dir.glob("market_data_prices_readonly_*.duckdb*"):
...
def get_connection():
    ...
    if not DB_PATH.exists():
        raise FileNotFoundError(
            f"Prices DuckDB not found at {DB_PATH}. "
            "Run ingest first or run the migration script."
        )
...
def _get_parquet_connection():
    ...
    close_connection()
    conn = duckdb.connect()
    register_parquet_views(conn, market="us", db_type="prices")
    _local.con = conn
