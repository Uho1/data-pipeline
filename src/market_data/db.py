"""DuckDB connection manager for market_data_lake (US main/financials).

Supports two storage backends (controlled by MDL_STORAGE env var):
  - "duckdb": file-based DuckDB (default, legacy)
  - "parquet": in-memory DuckDB with views over Parquet files
"""
...
DB_PATH: Path = DATA_DIR / "market_data.duckdb"

_local = threading.local()
_snapshot_lock = threading.Lock()
_snapshot_sig: tuple[int, int] | None = None
_SNAPSHOT_RE = re.compile(r"^market_data_lake_readonly_(\d+)\.duckdb(?:\..+)?$")
...
def _snapshot_path() -> Path:
    return Path(tempfile.gettempdir()) / f"market_data_lake_readonly_{os.getpid()}.duckdb"
...
def cleanup_stale_snapshot_copies() -> int:
    """Remove stale readonly snapshot copies left by dead processes."""
    removed = 0
    temp_dir = Path(tempfile.gettempdir())
    for candidate in temp_dir.glob("market_data_lake_readonly_*.duckdb*"):
...
def db_available() -> bool:
    """Return True if DuckDB is installed and data exists (DuckDB or Parquet)."""
    try:
        import duckdb  # noqa: F401
    except ImportError:
        return False
    if STORAGE_BACKEND == "parquet":
        return (PARQUET_DIR / "us").exists()
    return DB_PATH.exists()


def get_connection():
    """Return a thread-local DuckDB connection for reading.

    When MDL_STORAGE=parquet, returns an in-memory DuckDB with views over
    Parquet files. Otherwise uses the file-based DuckDB (with snapshot fallback).
    """
    import duckdb
...
def _get_parquet_connection():
    """Return an in-memory DuckDB connection with Parquet views (cached per-thread)."""
    import duckdb
    ...
    close_connection()
    conn = duckdb.connect()
    register_parquet_views(conn, market="us", db_type="main")
    _local.con = conn
